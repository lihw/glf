...
Model_3DS::~Model_3DS()
{
    for (int i = 0; i < numObjects; i++)
    {
        ...
        glDeleteBuffers(1, &(Objects[i].TexcoordBuffer));
        glDeleteVertexArrays(1, &(Objects[i].VertexArray));
...
void Model_3DS::InitGL()
{
    GLenum err;

    for (int i = 0; i < numObjects; i++)
    {
        glGenVertexArrays(1, &(Objects[i].VertexArray));

        glBindVertexArray(Objects[i].VertexArray);

        glGenBuffers(1, &(Objects[i].VertexBuffer));
        glGenBuffers(1, &(Objects[i].NormalBuffer));
        glGenBuffers(1, &(Objects[i].TexcoordBuffer));

        glBindBuffer(GL_ARRAY_BUFFER, Objects[i].VertexBuffer);
        glBufferData(GL_ARRAY_BUFFER, Objects[i].numVerts * 3 * sizeof(GLfloat), Objects[i].Vertexes, GL_STATIC_DRAW);
        glEnableVertexAttribArray(0);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, 0); 
        if ((err = glGetError()) != GL_NO_ERROR)
        {
            char msg[1024];
            sprintf(msg, "failed to create vertex buffer (%s)", gluErrorString(err));
            throw std::string(msg);
        }
        
        glBindBuffer(GL_ARRAY_BUFFER, Objects[i].NormalBuffer);
        glBufferData(GL_ARRAY_BUFFER, Objects[i].numVerts * 3 * sizeof(GLfloat), Objects[i].Normals, GL_STATIC_DRAW);
        glEnableVertexAttribArray(1);
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 0, 0); 
        if ((err = glGetError()) != GL_NO_ERROR)
        {
            char msg[1024];
            sprintf(msg, "failed to create normal buffer (%s)", gluErrorString(err));
            throw std::string(msg);
        }
        
        glBindBuffer(GL_ARRAY_BUFFER, Objects[i].TexcoordBuffer);
        glBufferData(GL_ARRAY_BUFFER, Objects[i].numTexCoords * 2 * sizeof(GLfloat), Objects[i].TexCoords, GL_STATIC_DRAW);
        glEnableVertexAttribArray(2);
        glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 0, 0); 
        if ((err = glGetError()) != GL_NO_ERROR)
        {
            char msg[1024];
            sprintf(msg, "failed to create texcoord buffer (%s)", gluErrorString(err));
            throw std::string(msg);
        }
    }
}
...
void Model_3DS::MapNameChunkProcessor(long length, long findex, int matindex)
{
    ...
    strcpy(Materials[matindex].texFilename, fullname);
