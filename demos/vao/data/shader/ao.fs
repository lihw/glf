...
void main()
{
    ...
    { 
        ...
        { 
            ...
            nodeIndex = imageLoad(StartOffsetBufferX, tc).x;
    ...
    //texcoord = pos.xy / pos.w * 0.5 + vec2(0.5, 0.5); 
    //// For those out of the frustum, we put them to the closest grid cell.
    //texcoord = texcoord * vec2(imageSize(StartOffsetBufferY));
    //nodeIndex = imageLoad(StartOffsetBufferY, ivec2(texcoord)).x;

    //while (nodeIndex != 0xffffffff)
    //{
    //    uvec4 p = imageLoad(GBufferY1, getImageCoord(nodeIndex, imageSize(GBufferX1)));
    //    uvec4 n = imageLoad(GBufferY2, getImageCoord(nodeIndex, imageSize(GBufferX2)));

    //    shadow -= 0.00001 * computeShadow(In.position, normalize(In.normal), 
    //                            vec3(uintBitsToFloat(p.x),
    //                                 uintBitsToFloat(p.y),
    //                                 uintBitsToFloat(p.z)),
    //                            vec3(uintBitsToFloat(n.x),
